...
class rute
{
    ...
    {
        $query = "INSERT INTO rute (nama_rute, jarak, harga) 
                  VALUES ('$nama_rute', '$jarak', '$harga')";
        $result = mysqli_query($this->koneksi, $query);
    ...
    {
        $query = "UPDATE rute SET nama_rute='$nama_rute', jarak='$jarak', harga='$harga' WHERE id_rute='$id_rute'";
        $result = mysqli_query($this->koneksi, $query);
        if ($result) {
            return true;
        } else {
